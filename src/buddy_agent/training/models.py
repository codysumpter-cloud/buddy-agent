...
class BuddyTrainingStats:
    """Small local-only growth stats for Buddy Training."""
    ...
    def clamp(self) -> None:
        """Clamp stats to the local progression range."""
        for name in STAT_NAMES:
            value = getattr(self, name)
            setattr(self, name, max(0, min(100, value)))

    def to_dict(self) -> dict[str, int]:
        """Serialize stats to plain JSON-compatible values."""
...
class BuddyTrainingState:
    ...
    This state is cosmetic/progression data only. It is not memory evidence, approval state,
    or proof that runtime work was completed.
    """
    ...
    def to_dict(self) -> dict[str, object]:
        """Serialize training state to a JSON-compatible object."""
        return {
    ...
    def from_dict(cls, data: dict[str, object]) -> "BuddyTrainingState":
        """Load training state from a JSON object."""
...
def _string_set(value: object) -> set[str]:
    """Return a set of strings from JSON-ish input."""
